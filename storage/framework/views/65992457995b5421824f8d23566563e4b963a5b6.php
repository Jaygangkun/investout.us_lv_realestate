        ...
        <table class="table table-striped table-hover" id="users">
            ...
            <tbody>
                <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr id="<?php echo e($user->profile); ?>">
                        <td class="client-avatar"><a href ="<?php echo e(route('profile.show',['user',$user->id])); ?>"><img alt="image" src="<?php echo e(asset('profilepic/'.$user->profile['image'])); ?> "> </a></td>
                        <td><?php echo e($user->first_name); ?></td>
                        <td><?php echo e($user->last_name); ?></td>
                        <td><?php echo e($user->profile['location']); ?></td>
                        <td><?php echo e($user->profile['zipCode']); ?></td>
                        <td><?php echo e($user->profile['phone']); ?></td>
                    
                        <?php if($user->last_login_at): ?>
                          <td><?php echo e($user->last_login_at->diffForHumans()); ?></td>
                        <?php else: ?>
                          <td></td>
                        <?php endif; ?>
                        <td><?php echo e(date('d-M-Y', strtotime($user->created_at))); ?></td>
                        <td><?php echo e($user->updated_at->diffForHumans()); ?></td>
                        <td  style="text-align: center;"><a href='#' onclick="getPackage( <?php echo e($user->id); ?> ); return false;"  class=""><i class = "fa fa-eye"></i></a></td>
                        <!--<?php if(file_exists('signed_documents/'.$user->id.'.pdf')): ?>
                            <td><a style="font-size:16px" target="_blank" href="<?php echo e(ENV('APP_URL').'signed_documents/'.$user->id.'.pdf'); ?>"><i class="fa fa-file-pdf-o"></i> View</a></td>
                        <?php else: ?>
                            <td>Not Signed</td>
                        <?php endif; ?>-->
                        ...
                        <?php else: ?>
                            <td>Active</td>
                            <td>
                                  <button type="submit" onclick="DeleteModal('<?php echo e(route('profile.delete',$user->id)); ?>')" class='btn btn-danger btn-sm' name="button">Delete <i class="fa fa-trash" aria-hidden="true"></i></button>
                            </td>
                        <?php endif; ?>
...
<script type="text/javascript">
    ...
    function getPackage(userid){
        ...
        $('.ViewFeatures').html("");
        $.ajax({
            url: "<?php echo e(ENV('APP_URL')); ?>/admin/stripe/get-user-plan-detail?userid="+userid,
            type: 'GET',
            dataType: 'json',
