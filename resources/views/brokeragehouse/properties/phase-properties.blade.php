    ...
    <div class="row animated fadeInRight allproperty_header">
      <div class="col-lg-12">
        <div class="ibox float-e-margins">
          ...
          @endif
          @if (session('add_property_exception'))
            <div class="alert alert-info">
              {{ session('add_property_exception') }}
            </div>
          @endif
            ...
            <div class="row m-t-sm animated fadeInRight">
              ...
              <div class="col-md-12">
                <div class="col-md-4" style="text-align: center;">
                  <input type="radio" name="filter" id="all" class="filter" value="0" checked=""> <label for="all"> All Properties </label>
                </div>
                <div class="col-md-4" style="text-align: center;">
                  <input type="radio" name="filter" id="realtors" class="filter" value="1"> <label for="realtors"> By Realtors </label>
                </div>
                <div class="col-md-4" style="text-align: center;" >
                  <input type="radio" name="filter" id="my" class="filter" value="2"> <label for="my"> My Properties </label>
                </div>
              </div>
                ...
                <div class="panel-body">
                  ...
                  <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="tab-1">
                      <a href="{{route('brokeragehouse.property.phase-index',0)}}"><button type="button" class='btn apply-button' @if($phase == 0) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">New Properties</button></a>
                      <a href="{{route('brokeragehouse.property.phase-index',1)}}"><button type="button" class='btn apply-button' @if($phase == 1) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Seller Validation</button></a>
                      <a href="{{route('brokeragehouse.property.phase-index',2)}}"><button type="button" class='btn apply-button' @if($phase == 2) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Evaluation</button></a>
                      <a href="{{route('brokeragehouse.property.phase-index',3)}}"><button type="button" class='btn apply-button' @if($phase == 3) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Title & liens Search</button></a>
                      <a href="{{route('brokeragehouse.property.phase-index',4)}}"><button type="button" class='btn apply-button' @if($phase == 4) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Property Market Evaluation</button></a>
                      <a href="{{route('brokeragehouse.property.phase-index',5)}}"><button type="button" class='btn apply-button' @if($phase == 5) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Approved</button></a>
                      <table class="table table-striped">
                        ...
                        <tbody>
                          ...
                          @if($property->userid != auth()->user()->id)
                            <tr class="allproperty_row RealtorsProperties all">
                          @else
                            <tr class="allproperty_row MyProperties all">
                          @endif
                            <?php
                              $url = ENV('APP_URL')."property-lists/".$property->id."/property-details"; 
                            ?>
                            <td><a  class='btn btn-primary' href="https://www.facebook.com/sharer/sharer.php?u={{$url}}" target="_blank">Share on Facebook</a></td>
                            <td class="text_center">{{$key+1}}</td>
                            <td class="client-avatar">
                              <a href="{{ route('brokeragehouse.property.show',$property->propertiesID) }}">
                                @php
                                  ...
                                  @if(!empty($images))
                                    @foreach($images as $img)
                                      @if($img['is_cover_image'] == 1) 
                                        @php $cover = 1;  @endphp
                                        <img alt="image" src="{{asset('properties/'.$property->propertiesID.'/images/'.$img['image'])}}"> 
                                      @endif
                                    @endforeach
                                    @if($cover == 0)
                                      @if(isset($property->images()->first()['image']) && $property->images()->first()['image'] != '')
                                        <img alt="image" src="{{asset('properties/'.$property->propertiesID.'/images/'.$property->images()->first()['image'])}}">
                                      @else
                            ...
                            @if($property->userid != auth()->user()->id && $property->is_submitted == 1)
                            <td>
                              <a href="{{ route('brokeragehouse.property.AcceptRejectProperty',['id'=>$property->propertiesID,'status'=>'2']) }}" title="Accept" id="acceptProp">
                                <i class="fa fa-check" style="font-size:21px;color: green;" aria-hidden="true"></i>
                              </a>
                              &nbsp;
                              <a realtor-id="{{$property->user_id}}" data-id="{{$property->propertiesID}}" href="#" title="Reject" id="rejectProp">
                                <i class="fa fa-times" style="font-size:21px;color: red;" aria-hidden="true"></i>
                              </a>
                            </td>
                            @else
                              @if($property->approved == 0)
                                <td>Pending</td>
                              @elseif($property->approved == 1)
                                  <td>Approved</td>
                              @endif
                            @endif
                            ...
                            <td><a onclick="DeleteModal('{{ route('DeleteProperty',['id'=>$phasenum, 'pid'=>$property->propertiesID]) }}');return false;" href="#" id="delProperty"><i class="fa fa-trash" style="font-size:21px;color: #36b394;" aria-hidden="true"></i></a></td>
                          </tr>
                          @endforeach
                        </tbody>
                      </table>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      $(document).on("click", "#acceptProp", function(){
        var checkstr =  confirm('Are you sure you want to accept this property?');
        if(checkstr == true){}
        else{
          return false;
        }
      });
      $(document).on("click", "#rejectProp", function(){
        var checkstr =  confirm('Are you sure you want to reject this property?');
        if(checkstr == true){
          $("#modalopenbutton").click();
          url = $(this).attr('href');
          id = $(this).attr('data-id');
          realtor_id = $(this).attr('realtor-id');

          $("#dataid").val(id);
          $("#realtor_id").val(realtor_id);
