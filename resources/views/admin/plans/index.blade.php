                ...
                <div class="ibox-title">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-right">
                            ...
                            <a href="{{ENV('APP_URL')}}/admin/stripe/sync-stripe-plans" class="btn btn-primary"><i class="fa fa-retweet"></i> Sync with stripe</a>
                        </div>
                    </div>
                </div>
                                        ...
                                        <table class="table" id="plans">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="allproperty_row">
                                                    ...
                                                    <td><?php echo env('APP_CURRRENCY').' ';?> {{$plan->amount}}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="javascript:void(0)" class="btn btn-info viewplan" data-stripeplan="{{$plan->plan_name}}" data-planid="{{$plan->plan_id}}" data-role="{{$plan->name}}" data-roleid="{{$plan->role_id}}" data-limitation="{{$limitation}}" data-createproperty="{{$plan->create_property}}" data-viewproperty="{{$plan->view_property}}" data-createrealtors="{{$plan->create_realtors}}"><i class="fa fa-eye"></i></a>
                                                        <a href="javascript:void(0)" class="btn btn-success editplan" data-id="{{$plan->id}}" data-planid="{{$plan->plan_id}}" data-planName="{{ $plan->plan_name }}" data-roleid="{{$plan->role_id}}" data-limitation="{{$plan->limitations}}" data-createproperty="{{$plan->create_property}}" data-viewproperty="{{$plan->view_property}}" data-createrealtors="{{$plan->create_realtors}}"><i class="fa fa-pencil"></i></a>
                                                        <label class="toggle-switch">
                                                            <input type="checkbox" <?php if($plan->status == 1){echo "checked";}?> data-id="{{$plan->id}}" class="plan_status">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="{{ route('admin.stripe.addUserPlans') }}" method="post" id="addUserPlanForm">
        {{ csrf_field() }}
        <div class="modal-body">
            <div class="row">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="{{ route('admin.stripe.updateUserPlan') }}" method="post" id="editUserPlanForm">
        {{ csrf_field() }}
        <div class="modal-body">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('.plan_status').change(function(){
            ...
            if(conf == 1){
                $.ajax({
                    url: "{{ENV('APP_URL')}}/admin/stripe/update-user-plan-status?id="+id+"&status="+status,
                    type: 'GET',
                    dataType: 'json',
        ...
        $('.editplan').click(function(){
            ...
            $.ajax({
                url: "{{ENV('APP_URL')}}/admin/stripe/get-user-plan?id="+planid,
                type: 'GET',
                dataType: 'json',
                success: function(res) {
                    stripeOptions = "";
                    ...
                    //var createproperty='{{-- $plan->create_property --}} data-viewproperty="{{-- $plan->view_property --}}" data-createrealtors'
    ...
    function updateview(roleid,planid){
        ...
        $.ajax({
            url: "{{ENV('APP_URL')}}/admin/stripe/get-role-features?id="+role+"&planid="+planid,
            type: 'GET',
            dataType: 'json',
            success: function(res) {
                var rolefeatures = "";
