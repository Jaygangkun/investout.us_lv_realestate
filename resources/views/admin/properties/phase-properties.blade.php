                ...
                <div class="panel-body">
                  @if (count($errors) > 0)
                    <div class="alert alert-danger">
                      <strong>Whoops!</strong> There were some problems while uploading images.<br><br>
                      <ul>
                  ...
                  <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="tab-1">
                      <a href="{{route('admin.property.phase-index',0)}}"><button type="button" class='btn apply-button' @if($phase == 0) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">New Properties</button></a>
                      <a href="{{route('admin.property.phase-index',1)}}"><button type="button" class='btn apply-button' @if($phase == 1) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Seller Validation</button></a>
                      <a href="{{route('admin.property.phase-index',2)}}"><button type="button" class='btn apply-button' @if($phase == 2) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Evaluation</button></a>
                      <a href="{{route('admin.property.phase-index',3)}}"><button type="button" class='btn apply-button' @if($phase == 3) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Title & liens Search</button></a>
                      <a href="{{route('admin.property.phase-index',4)}}"><button type="button" class='btn apply-button' @if($phase == 4) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Property Market Evaluation</button></a>
                      <a href="{{route('admin.property.phase-index',5)}}"><button type="button" class='btn apply-button' @if($phase == 5) style='background: #1ab394;border: 1px solid #1ab394;' @endif name="button">Approved</button></a>
                      <table class="table table-striped">
                        <thead>
                          <tr>
                            ...
                            <th>Building Type</th>
                            <!--<th>Is Active</th>-->
                            <th style='width:170px'>Acceptance</th>
                            @if ($phasenum == 4)
                            <th>Approve</th>
                            @else
                            <th>Update</th>
                        ...
                        <tbody>
                          ...
                          <tr class="allproperty_row">
                            ...
                            <td class="client-avatar">
                              <a href="{{ route('admin.property.show',$property->propertiesID) }}">
                                @php
                            ...
                            <td>{{$property->seller['first_name']." ".$property->seller['last_name']}}</td>
                            ...
                            <!--<td><input type="checkbox" {{ ($property->in_active == "0")? "checked" : "" }} data-toggle="toggle" data-style="{{$property->propertiesID}}" id="is-active" data-on="Yes" data-off="No"></td>-->
                            <td>
                              <form action="{{ route('admin.property.update',$property->propertiesID) }}" method="post" id='form{{ $key }}'>
                                {{ csrf_field() }}
                                <select class="form-control" name="acceptance_level">
                                    <?php
                                    $select_label = array("None","Seller Validation","Property Evaluation", "Titile&Lines Search","Property Market Evaluation","Approve");
                                    ?>
                                    @foreach ($select_label as $k => $label)
                                      <option value="{{ $k }}" {{ $k==$property->acceptance_level ? 'selected="selected"' : '' }}>{{ $label }}</option>
                                    @endforeach
                                  </select>
                              </form>
                            </td>
                            <td><button type="button" class='btn btn-primary form-submit' name="button" ref='{{ $key }}'><i class='fa fa-save'></i> Save</button></td>
                            ...
                            <td><a href="{{ route('admin.property.proposal.list',['property_id'=>$property->propertiesID]) }}" class="proposal-count-info"><i class="fa fa-list-alt" style="font-size:21px;color: #36b394;" aria-hidden="true"></i></a></td>
